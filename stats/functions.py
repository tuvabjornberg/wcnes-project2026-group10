                ...
                [
                    ...
                    min(
                        [
                            ...
                            np.float64(
                                np.float64(
                                    tmp * np.float64(math.cos(np.float64(two_pi * u2)))
                                )
                                + 0x1FFF
                            ),
                        ]
...
def compute_ber(df, PACKET_LEN=32, hamming_r=0):
    ...
    if len(df) > 0:
        errors, total = zip(
            *[
                compute_ber_packet(row, PACKET_LEN, hamming_r=hamming_r)
...
def radar_plot(metrics, system_ref, title):
    ...
    plt.plot(
        np.append(label_loc, (0.5 * np.pi)),
        system_ref + [system_ref[0]],
        label="Reference",
        color="grey",
    )
    ...
    lines, labels = plt.thetagrids(
        np.degrees(label_loc), labels=categories, fontsize=18
    )
